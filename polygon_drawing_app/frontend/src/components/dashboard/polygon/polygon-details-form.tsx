...
// Utility: point-in-polygon check (used for drawing grids)
...
// Component: renders polygon with draggable points and grid
const PolygonWithDraggablePoints = ({
    ...
    // Draw grid inside polygon (vertical + horizontal lines)
    const step = 20;
...
export function PolygonDetailsForm() {
    // States for image, zones, editing, notifications, selections
    ...
    // Fetch all cameras on load
    useEffect(() => {
        fetchData();
    }, []);

    // Fetch image + polygons when camera changes
    useEffect(() => {
        if (!selectedCameraSN) {
            setImage(null);
            setZones([]);
    ...
    // Keyboard shortcut: Delete key to remove points in edit mode
    useEffect(() => {
    ...
    // API call: fetch list of cameras
    ...
    // Save polygons to backend { /* PUT request */ }
    ...
    // Single delete { /* remove zone and update indices */ }
